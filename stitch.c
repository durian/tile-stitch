/*
  PJB 2020, added xplane mode.
 for z in 9 10 11 12;do ./stitch -c 56.289546 12.86720 1200 1200 $z http://a.tile.openstreetmap.org/{z}/{x}/{y}.png -x -o foo${z}.png;done
*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void usage(char **argv) {
    fprintf(stderr, "Usage: %s [-o outfile] [-x] [-f png|geotiff] minlat minlon maxlat maxlon zoom http://whatever/{z}/{x}/{y}.png ...\n", argv[0]);
    fprintf(stderr, "Usage: %s [-o outfile]  [-x] [-f png|geotiff] -c lat lon width height zoom http://whatever/{z}/{x}/{y}.png ...\n", argv[0]);
}
...
int main(int argc, char **argv) {
    ...
    while ((i = getopt(argc, argv, "o:t:cf:wx")) != -1) {
	switch (i) {
	case 'o':
	    outfile = optarg;
	    break;
	...
	if (outfmt == OUTFMT_PNG) {
	    ...
	    outmap = (char *) malloc(len + 1);
	    strcpy(outmap, outfile);
	    outmap[len - 3] = 'm';
	    outmap[len - 2] = 'a';
	    outmap[len - 1] = 'p';
	}
    ...
    if (dim > 16000 * 16000) {	// PJB increased
	...
	for (tx = tx1; tx <= tx2; tx++) {
		...
		*out = '\0';
	    ...
	    // Sleep here to avoid hammering server?
	    nanosleep((const struct timespec[]){{0, 500000000L}}, NULL);  // 0.5 seconds
	}
    }
    ...
    // Output for X-Trident map format.
    if (xplane) {
	/*
      BITMAP_NAME map_PAKT_z12.png
      WIDTH       2604
      HEIGHT      2603
      LON_WEST    -132.15492
      LON_EAST    -131.26098
    ...
    */
	FILE *mapfp = stdout;
	mapfp = fopen(outmap, "w");
	if (mapfp == NULL) {
	    perror(outmap);
	    exit(EXIT_FAILURE);
	}

	fprintf(mapfp, "BITMAP_NAME     %s\n", outfile);
	fprintf(mapfp, "WIDTH           %u\n", width);
	fprintf(mapfp, "HEIGHT          %u\n", height);
	fprintf(mapfp, "LON_WEST        %.6f\n", minlon);
	fprintf(mapfp, "LON_EAST        %.6f\n", maxlon);
	fprintf(mapfp, "LAT_SOUTH       %.6f\n", minlat);
	fprintf(mapfp, "LAT_NORTH       %.6f\n", maxlat);

	if (outfile != NULL) {
	    fclose(mapfp);
	}

	fprintf(stderr, "Output MAP: %s\n", outmap);
    }
